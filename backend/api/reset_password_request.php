...
if (!isset($data['email'])) {
    echo json_encode(["status" => "error", "message" => "Email обязателен"]);
    exit();
}
...
if (!$stmt_check) {
    error_log("Ошибка подготовки SELECT запроса: " . $conn->error);
...
if (!$stmt_update) {
    error_log("Ошибка подготовки UPDATE запроса: " . $conn->error);
    echo json_encode(["status" => "error", "message" => "Ошибка сервера"]);
    exit();
}
$stmt_update->bind_param("sss", $reset_token, $reset_expiry, $user_id);
if (!$stmt_update->execute()) {
    error_log("Ошибка выполнения UPDATE запроса: " . $stmt_update->error);
    echo json_encode(["status" => "error", "message" => "Ошибка сервера"]);
    exit();
}
$stmt_update->close();
...
if (sendResetEmail($email, $reset_link)) {
    echo json_encode(["status" => "success", "message" => "Ссылка на восстановление пароля отправлена на email"]);
} else {
    error_log("Ошибка при отправке email на: " . $email);
...
function sendResetEmail($email, $reset_link)
{
    ...
    try {
        ...
        $mail->Body = "<p>Для восстановления пароля перейдите по ссылке:</p><p><a href='$reset_link'>$reset_link</a></p>";
